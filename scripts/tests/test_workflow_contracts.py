...
class WorkflowContractTests(unittest.TestCase):
    ...
    def test_windows_msvc_195_compatibility_is_narrow_and_fail_closed(self) -> None:
        ...
        self.assertIn("Get-Item -LiteralPath $compilerPath -ErrorAction Stop", source)
        self.assertIn("Get-Item -LiteralPath $linkerPath -ErrorAction Stop", source)
        self.assertIn("$compilerFileVersion -notmatch '^14\\.51\\.'", source)
        self.assertIn("$linkerFileVersion -notmatch '^14\\.51\\.'", source)
        self.assertIn("$env:VCToolsVersion -notmatch '^14\\.51\\.'", source)
        self.assertIn("Set-Content -LiteralPath $probePath -Value 'DOBBY_MSVC_VERSION _MSC_VER'", source)
        self.assertIn("& $compilerPath /nologo /EP $probePath", source)
        self.assertIn("(?m)^DOBBY_MSVC_VERSION 1951\\r?$", source)
        self.assertIn('Add-Content -LiteralPath $env:GITHUB_ENV -Value "DOBBY_MSVC_COMPILER=$compilerPath"', source)
        self.assertIn('Add-Content -LiteralPath $env:GITHUB_ENV -Value "DOBBY_MSVC_LINKER=$linkerPath"', source)
        self.assertIn("Add-Content -LiteralPath $env:GITHUB_PATH -Value (Split-Path -Parent $compilerPath)", source)
        self.assertIn('-DCMAKE_C_COMPILER="$env:DOBBY_MSVC_COMPILER"', source)
        self.assertIn('-DCMAKE_CXX_COMPILER="$env:DOBBY_MSVC_COMPILER"', source)
        self.assertIn('-DCMAKE_LINKER="$env:DOBBY_MSVC_LINKER"', source)
        self.assertNotIn('-DCMAKE_C_COMPILER="cl.exe"', source)
        self.assertNotIn('-DCMAKE_CXX_COMPILER="cl.exe"', source)
        self.assertIn("Get-CMakeCachePath 'CMAKE_C_COMPILER'", source)
        self.assertIn("Get-CMakeCachePath 'CMAKE_CXX_COMPILER'", source)
